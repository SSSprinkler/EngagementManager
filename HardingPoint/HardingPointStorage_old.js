...
function writeFile(path,content) {
    return when.promise(function(resolve,reject) {
        HardingPointAPI.nodered_writeFile(path,content,function(error,data){
            if (error){
                console.log("nodered_writeFile Error : " + error);
                log.warn(log._("storage.remotefilesystem.hardingpoint.writeFile",{type:type}));
                reject(error);
            }
            else{
                console.log(data);
...
return when.promise(function(resolve) {
        fs.readFile(path,'utf8',function(err,data) {
            if (!err) {
                if (data.length === 0) {
                    log.warn(log._("storage.remotefilesystem.empty",{type:type}));
                    try {
                        ...
                        log.warn(log._("storage.remotefilesystem.restore",{path:backupPath,type:type}));
                        fs.copy(backupPath,path,function(backupCopyErr) {
                            if (backupCopyErr) {
                                // Restore backup failed
                                log.warn(log._("storage.remotefilesystem.restore-fail",{message:backupCopyErr.toString(),type:type}));
                                resolve([]);
                            } else {
                ...
                try {
                    ...
                    log.warn(log._("storage.remotefilesystem.invalid",{type:type}));
                    return resolve(emptyResponse);
                }
            } else {
                if (type === 'flow') {
                    log.info(log._("storage.remotefilesystem.create",{type:type}));
                }
...
function readFile(path,backupPath,emptyResponse,type) {
    return when.promise(function(resolve) {
        HardingPointAPI.nodered_readFile(path, function(err,data){
            if (!err) {
                try {
                    if (data.length === 0) {
...
var remotefilesystem = {
    ...
    getFlows: function() {
        if (!initialFlowLoadComplete) {
            ...
            log.info(log._("storage.remotefilesystem.user-dir",{path:settings.userDir}));
            log.info(log._("storage.remotefilesystem.flows-file",{path:flowsFullPath}));
        }
    ...
    getSettings: function() {
        return when.promise(function(resolve,reject) {
            HardingPointAPI.getfile(globalSettingsFile,function(err,data) {
                if (!err) {
                    try {
                        return resolve(parseJSON(data));
                    } catch(err2) {
    ...
    getSessions: function() {
        return when.promise(function(resolve,reject) {
            ...
            HardingPointAPI.getfile(sessionsFile,function(err,data) {
                if (!err) {
                    try {
                        return resolve(parseJSON(data));
                    } catch(err2) {
    ...
    getLibraryEntry: function(type, path) {
        ...
        return nodeFn.call(fs.lstat, rootPath).then(function(stats) {
            if (stats.isFile()) {
                return HardingPointAPI.nodered_readFile(rootPath, function(err,data){
                    if (!err){
                        return data;
                    }
                });
            ...
            if (type === "flows" && !/\.json$/.test(path)) {
                return remotefilesystem.getLibraryEntry(type,path+".json")
    ...
    saveLibraryEntry: function(type,path,meta,body) {
        ...
        return writeFile(path, headers+body, function(error,data){
            if (error){
