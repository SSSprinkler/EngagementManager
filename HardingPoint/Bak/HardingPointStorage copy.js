 ...
 * the write hits disk.
 */
function writeFile_old(path,content) {
    // Harding Point - Write to Cloud
    try{
        HardingPointAPI.nodered_writeFile(path,content,function(error,data){
            if (error){
                debug.outputiferror("HardingPointStorage.writeFile","HardingPointAPI.nodered_writeFile",error);
            }
        });
    }
    catch(ex){
        debug.outputiferror("HardingPointStorage.writeFile","HardingPointAPI.nodered_writeFile.exception",ex);
    }
...
function readFile(path,backupPath,emptyResponse,type) {
    return when.promise(function(resolve) {
        debug.write("HardingPointAPI.nodered_readFile : ",path,"");
...
function readFile_old(path,backupPath,emptyResponse,type) {
    return when.promise(function(resolve) {
